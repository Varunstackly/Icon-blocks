  ...
  return (
    <>
      ...
      <div className="flex flex-row flex-1 overflow-hidden h-full w-full relative">
        ...
        <div className={`
          ${isDesktop 
            ? 'static shrink-0 h-full w-[185px] translate-x-0 bg-[#0B1B3D]' 
            : `absolute inset-y-0 left-0 z-40 shrink-0 h-full w-[260px] transition-transform duration-300 ease-in-out bg-[#0B1B3D] ${isLeftOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-[100%]'}`
          }
        `}>
          <div className="h-full relative flex w-full">
            ...
            {!isDesktop && isLeftOpen && (
              <button 
                className="absolute top-4 right-4 bg-white/10 text-white p-1 rounded-full z-50 hover:bg-white/20 shadow-sm cursor-pointer"
        ...
        <div className={`
          ${isDesktop 
            ...
            : `absolute inset-y-0 right-0 z-40 shrink-0 h-full w-[260px] transition-transform duration-300 ease-in-out bg-[#FAF9F5] ${isRightOpen ? 'translate-x-0 shadow-[-10px_0_20px_rgba(0,0,0,0.1)]' : 'translate-x-[100%]'}`
          }
        `}>
          <div className="h-full relative flex flex-col items-start w-full">
            {!isDesktop && isRightOpen && (
              <button 
                className="absolute top-4 left-4 bg-black/5 text-[#55637A] p-1 rounded-full z-50 hover:bg-black/10 shadow-sm cursor-pointer"
